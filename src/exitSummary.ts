...
function dur(start: Date): string {
...
export function printExitSummary(sessionStart: Date, tokensUsed: number, user?: string): void {
  ...
  const apiRow = (label: string, value: string, labelColor: string, valueColor: string, indent = 0) => {
  ...
  const dualRow = (lLabel: string, lValue: string, rLabel: string, rValue: string, rLabelColor: string, rValueColor: string) => {
  ...
  for (const [m, n] of Object.entries(s.userByMethod)) {
    if (n > 0) apiRow(m, String(n), SAGE, SAGE, 2);
  }
