...
import { getTimePeriodRange, scanActivity, summarizeReport, type TimePeriod } from '../../core/domain/activity.js';
...
export function registerActivityCommand(program: Command): void {
  program
    ...
    .action(async (opts: { period: string; json?: boolean; force?: boolean }) => {
      const json = opts.json ?? false;
      const validPeriods: TimePeriod[] = ['24h', '7d', 'past-week', '30d', 'past-month'];

      if (!validPeriods.includes(opts.period as TimePeriod)) {
        process.stderr.write(`Error: --period must be one of: ${validPeriods.join(', ')}\n`);
        process.exit(1);
      }

      const period = opts.period as TimePeriod;
