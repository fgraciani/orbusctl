...
export function registerModelsCommand(program: Command): void {
  program
    ...
    .action(async (opts: { detail?: boolean; json?: boolean }) => {
      ...
      try {
        ...
        let counts: Map<string, { objects: number; relationships: number; drawings: number }> | undefined;
        if (opts.detail) {
        ...
        if (json) {
          const data = flat.map(({ model, depth, isLast: _isLast, ancestors: _ancestors }) => {
            const entry: {
              ...
              drawings?: number;
            } = { name: model.Name, modelId: model.ModelId, depth, isHidden: model.IsHidden };
            if (counts) {
          ...
          for (const { model, depth, isLast, ancestors } of flat) {
            ...
            if (counts) {
              const c = counts.get(model.ModelId);
              if (c) line += ` (${c.objects} obj, ${c.relationships} rel, ${c.drawings} drw)`;
            }
