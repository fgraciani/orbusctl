...
import {updateObject, updateObjectAttributes} from '../../api'
...
export default class ObjectsUpdate extends Command {
  static description = 'Update attribute values on an existing object'

  static enableJsonFlag = true

  static flags = {
    'object-id': Flags.string({description: 'Object ID (GUID)', required: true}),
  ...
  async run(): Promise<Record<string, unknown>> {
    const {flags} = await this.parse(ObjectsUpdate)
    ...
    this.log(`Updating object ${flags['object-id']}...`)
    ...
    if (flags['set-choice']) {
      ...
      const params = {objectId: flags['object-id'], attributeValues}
      try {
        result = await updateObjectAttributes(token, flags['object-id'], attributeValues) as Record<string, unknown>
      } catch (err) {
        logWrite({operation: 'updateObject', modelId: '', params, success: false, error: (err as Error).message, user: getUser()})
        throw err
      }
      logWrite({operation: 'updateObject', modelId: '', params, success: true, result, user: getUser()})
      this.log(`Updated object: ${flags['object-id']}`)
      return {attributeValues, objectId: flags['object-id'], result}
    }
    ...
    try {
      result = await updateObject(token, flags['object-id'], attributes) as Record<string, unknown>
    } catch (err) {
      logWrite({operation: 'updateObject', modelId: '', params: {objectId: flags['object-id'], attributes}, success: false, error: (err as Error).message, user: getUser()})
      throw err
    }

    logWrite({operation: 'updateObject', modelId: '', params: {objectId: flags['object-id'], attributes}, success: true, result, user: getUser()})
