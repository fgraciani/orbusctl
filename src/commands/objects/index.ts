...
import {fetchDrawingCount, fetchDrawingsContainingObject, fetchModel, fetchModels, fetchObjectDetail, fetchObjectModelName, fetchObjectRelationships, fetchObjects} from '../../api'
...
export default class Objects extends Command {
  ...
  static flags = {
    ...
    'object-id': Flags.string({description: 'Object ID (GUID) — use instead of --object'}),
  }
  ...
  async run(): Promise<Record<string, unknown>> {
    ...
    if (flags.object && flags['object-id']) {
      this.error('Provide --object or --object-id, not both.')
    }
    ...
    if (flags['object-id']) {
      this.log(`Fetching details for object "${flags['object-id']}"...`)
      this.log()
      return this.fetchObjectDetailFull(token, flags['object-id'], match.ModelId, modelRef)
    }
    ...
    if (flags.object) {
      ...
      this.log()
      return this.fetchObjectDetailFull(token, obj.ObjectId, match.ModelId, modelRef)
    }
  ...
  private async fetchObjectDetailFull(
    token: string,
    objectId: string,
    modelId: string,
    modelRef: {modelId: string; name: string},
  ): Promise<Record<string, unknown>> {
    const detail = await fetchObjectDetail(token, objectId)
    ...
    const [relationships, drawings] = await Promise.all([
      ...
      fetchDrawingsContainingObject(token, modelId, objectId),
    ])
