...
import type { Section, ModelsView } from '../types.js';
...
function getHelpLines(props: HelpPanelProps): [string, string] {
  const {
    section, modelsView, viewedObjectId,
    viewedDrawingId, viewedDrawingObjectId,
    auditMode, configEditing,
    activityHasReport, activityScanning, activityModelIndex,
    exportModelId, exportDone, exportInProgress,
  } = props;
...
export const HelpPanel: React.FC<HelpPanelProps> = (props) => {
  ...
  return (
    <Panel title="Help" focused={false} flexGrow={1}>
      <Box paddingX={1} flexDirection="column">
        <Box><Text color="gray">{line1}</Text></Box>
        <Box><Text color="gray" dimColor>{line2}</Text></Box>
      </Box>
    </Panel>
  );
};
