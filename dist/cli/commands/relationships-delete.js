...
export function registerRelationshipsDeleteCommand(program) {
    program
        ...
        try {
            ...
            let objectName = '';
            try {
                const rel = await odata(token, `${getBaseUrl()}/odata/Relationships(${opts.relationshipId})?$expand=RelationshipType($select=Name),LeadObject($select=Name),MemberObject($select=Name)`);
