...
import { saveCorrelationTable } from '../../core/domain/correlation.js';
...
export function registerObjectsMoveCommand(program) {
    program
        ...
        try {
            ...
            const intraRels = relationships.filter(r => r.LeadObject?.ObjectId && r.MemberObject?.ObjectId &&
            ...
            for (const rel of intraRels) {
                if (!rel.RelationshipType || !rel.LeadObject || !rel.MemberObject)
                    continue;
                ...
                try {
                    const result = await createRelationship(token, opts.targetId, relTypeId, rel.LeadObject.ObjectId, rel.MemberObject.ObjectId, copyAttrs.length > 0 ? copyAttrs : undefined);
...
function extractRelId(result) {
    if (!result || typeof result !== 'object')
        return undefined;
    const r = result;
    const msg = r['successMessage'];
    const def = msg?.['messageDefinition'];
    if (def?.['relationshipId'])
        return String(def['relationshipId']);
    if (r['RelationshipId'])
        return String(r['RelationshipId']);
    if (r['relationshipId'])
        return String(r['relationshipId']);
    return undefined;
}
