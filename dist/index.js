...
import { SECTIONS, SECTIONS_REQUIRING_AUTH } from './types.js';
import { getToken, getUser, saveAuth, hasToken, saveSolutionFilter, getShowHiddenModels, saveShowHiddenModels, getServer, saveServer, getWritePasswordHash, saveWritePassword, saveBrowser } from './core/config.js';
...
export const App = (props) => {
    ...
    const [focusedPanel, setFocusedPanel] = useState('sidebar');
    ...
    const { exit } = useApp();
    const [modelsView, setModelsView] = useState({ level: 'list' });
    const [viewedObjectId, setViewedObjectId] = useState(null);
    const [detailFocused, setDetailFocused] = useState(false);
    const [relIndex, setRelIndex] = useState(0);
    const [drawingsModelId, setDrawingsModelId] = useState(null);
    const [drawingsModelName, setDrawingsModelName] = useState('');
    const [viewedDrawingId, setViewedDrawingId] = useState(null);
    const [viewedDrawingObjectId, setViewedDrawingObjectId] = useState(null);
    const [drawingDetailFocused, setDrawingDetailFocused] = useState(false);
    const [drawingRelIndex, setDrawingRelIndex] = useState(0);
    const [viewedAuditObjectId, setViewedAuditObjectId] = useState(null);
    ...
    const [exportTemplateVars, setExportTemplateVars] = useState([]);
    const [exportVarValues, setExportVarValues] = useState({});
    const [exportVarIndex, setExportVarIndex] = useState(-1);
    const [exportVarInput, setExportVarInput] = useState('');
    const [exportVarOptions, setExportVarOptions] = useState([]);
    const [exportVarLoading, setExportVarLoading] = useState(false);
    const [compareModelAId, setCompareModelAId] = useState(null);
    const [compareModelAName, setCompareModelAName] = useState('');
    const [compareModelBId, setCompareModelBId] = useState(null);
    ...
    const [auditIssueFilter, setAuditIssueFilter] = useState(null);
    const [auditMode, setAuditMode] = useState('menu');
    const [auditSelectedModelIds, setAuditSelectedModelIds] = useState(new Set());
    const [exportModelId, setExportModelId] = useState(null);
    ...
    const [configEditing, setConfigEditing] = useState(null); // which setting is being edited
    const [configTextInput, setConfigTextInput] = useState('');
    const activity = useActivity();
    const [activityModelIndex, setActivityModelIndex] = useState(null);
    const TIME_PERIODS = ['24h', '7d', 'past-week', '30d', 'past-month'];
    const modelFlat = useMemo(() => models.length > 0 ? flattenTree(buildTree(models)) : [], [models]);
    useEffect(() => {
    ...
    const maxMainItems = (() => {
        ...
        switch (activeSection) {
            ...
            case 'audit':
                if (auditMode === 'menu')
                    return 3;
                if (auditMode === 'select-model')
                    return modelFlat.length;
                if (auditMode === 'select-models')
                    return modelFlat.length;
                if (auditMode === 'detail' && audit.result && auditIssueFilter)
                    return audit.result.issues.filter(i => i.issueType === auditIssueFilter).length;
                if (auditMode === 'detail' && audit.result)
                    return Object.keys(audit.result.issuesByType).length;
                if (auditMode === 'all-results')
                    return modelFlat.length;
                if (audit.auditing || audit.scanningAll)
                    return 0;
                return 0;
            case 'compare':
                if (compareModelBId)
                    return compare.rows.length;
                return modelFlat.length; // selecting model A or B
            case 'export':
                if (templateExport.exporting || templateExport.result || templateExport.error)
                    return 0;
                if (exportHook.result || exportHook.exporting)
                    return 0;
                if (exportVarIndex >= 0)
                    return exportVarOptions.length > 0 ? exportVarOptions.length : 0;
                if (exportTemplatePicking)
                    return exportTemplates.length;
                if (exportModelId)
                    return EXPORT_FORMATS.length;
                return modelFlat.length;
            case 'config':
                if (configEditing === 'solution')
            ...
            case 'activity':
                ...
                if (activity.report)
                    return activity.report.models.length;
    ...
    useInput((input, key) => {
        ...
        if (key.downArrow || input === 'j') {
            ...
            if (activeSection === 'models') {
                ...
                if (auditMode === 'menu') {
                    if (mainItemIndex === 0) {
                        setAuditMode('select-model');
                        setMainItemIndex(0);
                    }
                    else if (mainItemIndex === 1 && realToken) {
                        audit.startScanAll(realToken, modelFlat.map(e => e.model));
                        setAuditMode('all-results');
                        setMainItemIndex(0);
                    }
                    else if (mainItemIndex === 2) {
                        setAuditMode('select-models');
                        setAuditSelectedModelIds(new Set());
                        setMainItemIndex(0);
                    }
                }
                    ...
                    if (types[mainItemIndex]) {
                        setAuditIssueFilter(types[mainItemIndex]);
                        setMainItemIndex(0);
                    }
                }
                else if (auditMode === 'detail' && audit.result && auditIssueFilter) {
                ...
                if (!exportModelId && modelFlat[mainItemIndex]) {
                    ...
                    if (fmt?.value === 'template') {
                        ...
                        exportHook.startExport(realToken, exportModelId, exportModelName, fmt.value, true);
                    }
                }
            }
    ...
    return (_jsxs(Box, { flexDirection: "column", width: "100%", height: termSize.rows, children: [showExitConfirm && (_jsx(ConfirmModal, { sessionStart: sessionStart, tokensUsed: tokensUsed, user: auth.user, onConfirm: () => { props.onStoreExitData(sessionStart, tokensUsed, auth.user); exit(); }, onCancel: () => setShowExitConfirm(false) })), !showExitConfirm && showModal && (_jsx(WelcomeModal, { auth: auth, onAuthenticated: handleAuthenticated, onSkip: handleSkip, onDismiss: () => setShowModal(false) })), _jsxs(Box, { display: modalActive ? 'none' : 'flex', flexDirection: "column", width: "100%", flexGrow: 1, children: [_jsx(Header, { auth: auth, heartbeatStatus: heartbeatStatus, heartbeatLastCheck: heartbeatLastCheck }), _jsxs(Box, { flexGrow: 1, alignItems: "stretch", children: [_jsx(Sidebar, { sectionIndex: sectionIndex, focused: focusedPanel === 'sidebar', auth: auth }), _jsx(MainContent, { section: activeSection, selectedIndex: mainItemIndex, focused: focusedPanel === 'main', auth: auth, models: models, modelFlat: modelFlat, modelCounts: modelCounts, modelsLoading: modelsLoading, modelsError: modelsError, modelsView: modelsView, objects: objects, objectsLoading: objectsLoading, objectsError: objectsError, objectDetail: objectDetail, objectRelationships: objectRelationships, detailLoading: detailLoading, detailError: detailError, viewedObjectId: viewedObjectId, detailFocused: detailFocused, relIndex: relIndex, drawings: drawings, drawingTypeMap: drawingTypeMap, drawingsLoading: drawingsLoading, drawingsError: drawingsError, drawingsModelId: drawingsModelId, drawingsModelName: drawingsModelName, viewedDrawingId: viewedDrawingId, drawingComponents: drawingComponents, drawingDetailLoading: drawingDetailLoading, drawingDetailError: drawingDetailError, viewedDrawingObjectId: viewedDrawingObjectId, drawingObjectDetail: drawingObjectDetail, drawingObjectRelationships: drawingObjectRelationships, drawingObjectDetailLoading: drawingObjectDetailLoading, drawingObjectDetailError: drawingObjectDetailError, drawingDetailFocused: drawingDetailFocused, drawingRelIndex: drawingRelIndex, termRows: termSize.rows - (showHelp ? 4 : 0), termCols: termSize.cols, showStats: showStats, activity: activity, activityPeriods: TIME_PERIODS, activityModelIndex: activityModelIndex, audit: audit, auditIssueFilter: auditIssueFilter, auditMode: auditMode, auditSelectedModelIds: auditSelectedModelIds, viewedAuditObjectId: viewedAuditObjectId, auditObjectDetail: auditObjectDetail, auditObjectRelationships: auditObjectRelationships, auditDetailLoading: auditDetailLoading, auditDetailError: auditDetailError, auditDetailFocused: auditDetailFocused, auditRelIndex: auditRelIndex, compare: compare, compareModelAId: compareModelAId, compareModelAName: compareModelAName, compareModelBId: compareModelBId, compareModelBName: compareModelBName, auditExport: auditExport, activityExport: activityExport, exportHook: exportHook, templateExport: templateExport, exportTemplatePicking: exportTemplatePicking, exportTemplates: exportTemplates, exportTemplate: exportTemplate, exportTemplateVars: exportTemplateVars, exportVarIndex: exportVarIndex, exportVarInput: exportVarInput, exportVarOptions: exportVarOptions, exportVarLoading: exportVarLoading, exportModelId: exportModelId, exportFormats: EXPORT_FORMATS, configEditing: configEditing, configTextInput: configTextInput, solutions: solutions }), _jsx(Box, { display: showStats ? 'flex' : 'none', children: _jsx(StatsPanel, { auth: auth, sessionStart: sessionStart, heartbeatLatency: heartbeatLatency, apiTotal: apiTotal, apiStartup: apiStartup, apiHeartbeat: apiHeartbeat, apiUserByMethod: apiUserByMethod, chartBuckets: chartBuckets, chartActiveMethods: chartActiveMethods }) })] }), _jsx(Box, { display: showHelp ? 'flex' : 'none', children: _jsx(HelpPanel, { section: activeSection, modelsView: modelsView, viewedObjectId: viewedObjectId, viewedDrawingId: viewedDrawingId, viewedDrawingObjectId: viewedDrawingObjectId, auditMode: auditMode, configEditing: configEditing, activityHasReport: activity.report !== null, activityScanning: activity.scanning, activityModelIndex: activityModelIndex, exportModelId: exportModelId, exportDone: exportHook.result !== null, exportInProgress: exportHook.exporting }) }), _jsx(Footer, { focusedPanel: focusedPanel, section: activeSection, showStats: showStats })] })] }));
};
