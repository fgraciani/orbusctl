...
export async function scanActivity(token, models, since, until, label, onProgress) {
    ...
    for (let i = 0; i < models.length; i++) {
        ...
        try {
            ...
            if (objects.length > 0)
                objectsByModel.set(model.ModelId, objects);
            if (relationships.length > 0)
                relationshipsByModel.set(model.ModelId, relationships);
            onProgress?.({
                current: i + 1,
                total: models.length,
...
export function summarizeReport(report) {
    ...
    for (const model of report.models) {
        ...
        if (objects.length === 0 && rels.length === 0)
            continue;
        const userMap = new Map();
        const userRels = new Map();
        for (const obj of objects) {
            ...
            if (!userMap.has(userName))
                userMap.set(userName, { created: [], modified: [] });
            const entry = userMap.get(userName);
            if (isNew) {
                entry.created.push(obj);
                totalCreated++;
            }
            else {
                entry.modified.push(obj);
                totalModified++;
            }
        }
        ...
        const users = [...allUsers].sort().map(userName => ({
            userName,
