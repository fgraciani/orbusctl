...
export async function performExcelExport(token, modelId, modelName, fetchDetails, onProgress) {
    ...
    if (details) {
        ...
        for (const d of details.values()) {
            for (const a of d.AttributeValues) {
                if (!SYSTEM_ATTRS.has(a.AttributeName) && a.StringValue)
                    attrNames.add(a.AttributeName);
            }
        }
    ...
    const relAttrNames = new Set();
    for (const r of relationships) {
        for (const a of r.AttributeValues ?? []) {
            if (a.StringValue && !SYSTEM_REL_ATTRS.has(a.AttributeName))
                relAttrNames.add(a.AttributeName);
        }
    }
