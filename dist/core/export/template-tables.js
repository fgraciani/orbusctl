...
export function sanitizeDiagramName(name) {
    return name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
}
export function getRelAttrValue(rel, attrName) {
    const attr = rel.AttributeValues?.find((a) => a.AttributeName === attrName);
    return attr?.StringValue ?? attr?.Value ?? '';
}
export function findProcessByName(name, objectsById) {
    for (const obj of objectsById.values()) {
        if (obj.type === 'Business process' && obj.name === name)
            return obj;
    }
    return undefined;
}
export function getChildProcesses(parentId, relationships, objectsById, seen) {
    const visited = seen ?? new Set();
    const results = [];
    for (const rel of relationships) {
        if (rel.RelationshipType?.Name !== 'ArchiMate: Aggregation')
            continue;
        if (rel.LeadObject?.ObjectId !== parentId)
            continue;
...
export function resolveTasks(processName, process, scopeOverride, relationships, objectsById) {
...
export function findActivityForTask(taskId, relationships, objectsById) {
    const parentRels = relationships.filter((r) => r.RelationshipType?.Name === 'ArchiMate: Aggregation' && r.MemberObject?.ObjectId === taskId);
    if (parentRels.length === 0)
        return '(unknown)';
    for (const rel of parentRels) {
        const parent = objectsById.get(rel.LeadObject?.ObjectId ?? '');
        if (parent?.type === 'Business process')
            return parent.name;
    }
    for (const rel of parentRels) {
        const parent = objectsById.get(rel.LeadObject?.ObjectId ?? '');
        if (parent?.type === 'Grouping') {
            const gpRels = relationships.filter((r) => r.RelationshipType?.Name === 'ArchiMate: Aggregation' && r.MemberObject?.ObjectId === parent.id);
            for (const grel of gpRels) {
                const gp = objectsById.get(grel.LeadObject?.ObjectId ?? '');
                if (gp?.type === 'Business process')
                    return gp.name;
            }
        }
    }
...
export function resolveTasksFromAttr(attrs, objectsById) {
...
export function formatRasciShort(stringValue) {
    if (!stringValue)
        return '';
    return stringValue
        .split(',')
...
export function generateTasksTable(attrs, relationships, objectsById, scopeOverride) {
    ...
    if (!processName)
        return `<!-- ERROR: tasks table requires process="..." or tasks="..." -->`;
    const process = findProcessByName(processName, objectsById);
    if (!process)
        return `<!-- ERROR: Process "${processName}" not found in model -->`;
    const tasks = [...resolveTasks(processName, process, scopeOverride, relationships, objectsById)].sort((a, b) => a.name.localeCompare(b.name));
    const lines = ['', '| Task | Description |', '|---|---|'];
...
export function generateIOTable(attrs, relationships, objectsById, scopeOverride) {
    const processName = attrs['process'];
    const direction = attrs['direction'];
    if (!processName)
        return `<!-- ERROR: io table requires process="..." -->`;
    ...
    if (!process)
        return `<!-- ERROR: Process "${processName}" not found in model -->`;
    const scopeIds = new Set([process.id]);
    for (const task of resolveTasks(processName, process, scopeOverride, relationships, objectsById))
        scopeIds.add(task.id);
    const collected = new Map();
    for (const rel of relationships) {
        if (rel.RelationshipType?.Name !== 'ArchiMate: Access')
            continue;
        ...
        if (!processIsLead && !processIsMember)
            continue;
    ...
    for (const obj of sorted)
        lines.push(`| ${cell(obj.name)} | ${cell(obj.description)} |`);
    lines.push('');
    return lines.join('\n');
}
export function generateRolesTable(attrs, relationships, objectsById, scopeOverride) {
    const processName = attrs['process'];
    let scopeIds;
    if (processName) {
        const process = findProcessByName(processName, objectsById);
        ...
        scopeIds = new Set();
        for (const obj of objectsById.values()) {
            if (obj.type === 'Business process')
                scopeIds.add(obj.id);
        }
    }
    const collected = new Map();
    for (const rel of relationships) {
        if (rel.RelationshipType?.Name !== 'ArchiMate: Association')
            continue;
        const leadId = rel.LeadObject?.ObjectId;
        const memberId = rel.MemberObject?.ObjectId;
        if (!leadId || !memberId)
    ...
    for (const role of sorted)
        lines.push(`| ${cell(role.name)} | ${cell(role.description)} |`);
    lines.push('');
    return lines.join('\n');
}
export function generateRasciTable(attrs, relationships, objectsById, scopeOverride) {
    const processName = attrs['process'];
    if (processName) {
        const process = findProcessByName(processName, objectsById);
        if (!process)
        ...
        for (const rel of relationships) {
            ...
            if (roleId && taskId) {
                roleSet.add(roleId);
                if (!rasciMap.has(taskId))
                    rasciMap.set(taskId, new Map());
    ...
    if (rasciRels.length === 0)
        return `<!-- ERROR: No RASCI associations found -->`;
    ...
    for (const { activity, obj: task } of tasksSorted) {
        ...
        lines.push(`| ${cell(activity)} | ${cell(task.name)} | ${rolesSorted.map((r) => row.get(r.id) ?? '').join(' | ')} |`);
    }
    lines.push('');
    return lines.join('\n');
}
export function generateLifecycleList(attrs, relationships, objectsById) {
    const processName = attrs['process'];
    if (!processName)
        return `<!-- ERROR: lifecycle table requires process="..." -->`;
    const process = findProcessByName(processName, objectsById);
    if (!process)
        return `<!-- ERROR: Process "${processName}" not found in model -->`;
    const direct = [];
    for (const rel of relationships) {
        if (rel.RelationshipType?.Name !== 'ArchiMate: Aggregation')
            continue;
        if (rel.LeadObject?.ObjectId !== process.id)
            continue;
        const memberId = rel.MemberObject?.ObjectId;
        if (!memberId)
            continue;
...
export function generateDiagram(attrs, assetRelPath, assetAbsDir) {
...
export function processTag(tagType, attrStr, relationships, objectsById, scopeOverride, assetRelPath, assetAbsDir) {
